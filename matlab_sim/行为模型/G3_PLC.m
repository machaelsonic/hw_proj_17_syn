...
first_carrier_id=111;%起始子载波编号
last_carrier_id=238;%终止子载波编号

carrier_num=128;%使用的子载波个数
fft_point=1024;% FFT点数


phase_data=load('E:\design\QUARTUS\plc_design_final.git\matlab_sim\行为模型\phase.txt'); %从文件中读取前导序列相位
...
for frame_id=1:frame_num
    ...
    tx_data=[pre_data*16 send_data_ifft_r_rtl*2]; %发送到电力线上的数据， 共16564个采样点,对前导序列放大4倍,实际设计过程中可根据具体数值进行调整
